...
static func get_beam_count(level: int) -> int:
	return 3 + int(level / 3)

# 伤害值配置
# LV1=120, LV10=150（原版数据，每级+6）
static func get_damage(level: int) -> float:
	return base_damage + level * 6.0

# 魔法消耗配置
# LV1=35, LV10=54（原版数据，每级+1.9）
static func get_mana_cost(level: int) -> float:
	return base_mana_cost + level * 1.9
...
static func cast(hero: Node, mouse_pos: Vector2, skill_cooldowns: Dictionary) -> bool:
	...
	if Global.free_spells or Global.mana >= mana_cost:
		...
		# 扇形分布配置
		var spread_angle = deg_to_rad(30.0)        # 总扇形角度：30度
		...
		for i in range(beam_count):
			...
			# 投射物参数配置
			beam.speed = 1000.0                      # 飞行速度：2000（极快）
			beam.max_distance = 600.0                # 最大射程：600像素
