...
> **语言**: GDScript
> **最后更新**: 2026-05-16（v11: 加密导出完成 + 输入处理教训文档化）
...
| 6 | **Diablo** (召唤者) | 召唤 | 不直接攻击，每 5 秒召唤 4 只怪物 | 35 |
...
- **地图大小**: 1536 × 1536 像素（v6调整，=1024×1.5，与原版速度一致）
- **摄像机**: zoom = 1.0（还原原版视野比例）
- **安全边界**: 80px（怪物生成在边界内侧，避免卡墙）
- **生成 padding**: 100px（怪物从画面外生成走入视野）
- **英雄出生点**: (768, 768) — 地图中心

### 6.2 生成模式类型

原版游戏有四种独立定时器的生成模式（来源于反编译理解和用户确认）：

```gdscript
enum SpawnPattern { SINGLE, LINE, GROUP, ALL_SIDES }
```

> ⚠️ **注意**：以下各模式的参数（触发间隔、生成数量、解锁等级）**来源不确定**。
> Data.pak 全量解密（92个文件）中**没有找到**对应的配置文件。
> 这些数值基于之前开发者的反编译理解和用户对原版游戏的记忆。
> **建议根据实际游戏体验调整**。
...
每种怪物有基础权重，生成时只计算当前等级**可用**的怪物权重总和，归一化概率（来源不确定）：
...
- **v7 已移除上限**：原 `max_monsters=15` 的硬上限已彻底删除
- 所有4种生成模式完全独立自由运转
- 唯一限制：Diablo 最多3只同时在场
